...
def multi_feature2terr_zones_initial(task, **kwargs):
    ...
    feature_centroids = initial_gdf.geometry.representative_point()
    proxy_zone_geom = proxy_zones.set_index("zone")["geometry"].to_dict()
    zone_anchors: dict = {}
    for z in terr_zone_list:
        if z in fixed_zones_in_poly:
            zone_anchors[z] = fixed_zones_in_poly[z]
        else:
            geom = proxy_zone_geom.get(z)
    ...
    for z in terr_zones["zone"]:
        model += (
            pulp.lpSum(x[i, z] for i in zone_capacity.keys() if (i, z) in x) >= target_areas[z],
            f"TargetArea_{z}",
        )
    ...
    model.solve(pulp.PULP_CBC_CMD(msg=False, timeLimit=20, gapRel=0.05))
    ...
    if model.status == pulp.LpStatusInfeasible:
        ...
        kwargs.update({"infeasible": kwargs.get("infeasible", 0) + 1})
